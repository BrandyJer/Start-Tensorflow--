...
max_iters = 10000
TFrecord_path = r"/root/linjian/darknet_patch/TF_data/train"
#width_patch = 256
#height_patch = 180
#for code test
#max_iters = 100
...
train_queue_list = os.listdir(TFrecord_path)
for i in train_queue_list:
    temp_path = os.path.join(TFrecord_path, i)
    train_queue.append(temp_path)
...
#6-27            train_queue_list = os.listdir(TFrecord_path)
#|            for i in train_queue_list:
#|                temp_path = os.path.join(TFrecord_path, i)
#6-27                train_queue.append(temp_path)
...
###################################################################
            ...
            opt = tf.train.MomentumOptimizer(0.15,0.9)
...
#            opt.minimize(total_loss, global_step=global_step)
            ...
            train_writer = tf.summary.FileWriter('/root/linjian/darknet_patch/models/0_wd4e5-0.15', sess.graph)
            sess.run(init)
...
#                saver.restore(sess, model_file)
                model_file=tf.train.get_checkpoint_state('./models/0_wd4e5-0.15')
...
#                    print("The accuracy is :"+str(accuracy))
                    acc_batch = 0.0
                    loss_cnt = 0.0
                if step % 100 == 0 or (step + 1) == max_iters:
#                if step % 50 == 0 or (step + 1) == max_iters:
#                    checkpoint_path = os.path.join('/root/classify/model', 'dp15_model.ckpt')
                    checkpoint_path = os.path.join('/root/linjian/darknet_patch/models/0_wd4e5-0.15', 'model.ckpt')#save model path
