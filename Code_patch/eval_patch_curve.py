...
#            train_writer = tf.summary.FileWriter('/root/linjian/darknet_patch/models/dp0.3_lr0.001', sess.graph)
#           ###################################################
            eval_queue = "../TF_data/validation.tfrecords"
            ...
            sess = tf.Session()
            train_writer = tf.summary.FileWriter('/root/linjian/darknet_patch/models/0_wd4e5-0.15/eval', sess.graph)
            sess.run(init)

            tf.train.start_queue_runners(sess=sess)

#            ckpt = tf.train.get_checkpoint_state("/root/JZ_test/darknet0_model")
            ckpt = tf.train.get_checkpoint_state("/root/linjian/darknet_patch/models/0_wd4e5-0.15")
#####            saver.restore(sess, ckpt.all_model_checkpoint_paths[-1])
#####            step = 0 
            for model in range(-10, 0):    #-55==>model-2000
#####            step+=1
...
#####                print("The "+str(step)+" iter eval loss:"+str(mse/float(num)))
                ...
                print("========Thre :"+str(thre)+'/'+str(model+10)+" _model eval ========")
                ...
                train_writer.add_summary(summary_str, model+10)
            train_writer.close()
