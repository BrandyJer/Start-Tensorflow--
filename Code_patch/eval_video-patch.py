...
from sliding_window import output_patch
import subprocess
import shutil
...
if __name__ == '__main__':
    ...
    HP_OutPath = videoFilePath+"diffmpeg_116475395_0"
    start_time = 0
    end_time = 6*60
    shell_command_0 = r"ffmpeg -i "+videoFilePath
#    shell_command_0 = shell_command_0+r"116475395_0.mkv -s 320x180 -sws_flags bilinear "
    shell_command_0 = shell_command_0+r"116475395_0.mkv "
    videoFile = cv2.VideoCapture(videoFilePath+'116475395_0.mkv')
            ...
            if fps_jpg%25 == 0 and (len(diff_1s_list)>0):   #every 1s ==every 25 jpgs
                try:
                    ...
                    for top3 in range(top):   #every top3 init img
                        ...
                        ##############output_patch##############
                        patch_list, all_windows = output_patch(diff_1s_list[top3][1])
                        for patch in patch_list:   # one img
                            patch_tf = np.expand_dims(patch, 0)
                            HP_patch, HP, YT =  eval_video(256, 180, patch_tf)
                            if HP_patch == 1:
                                HP_patch_num += 1
