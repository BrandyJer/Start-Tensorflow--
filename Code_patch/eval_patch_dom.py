...
#            tf.train.start_queue_runners(sess=sess)
            ckpt = tf.train.get_checkpoint_state(r"/root/linjian/darknet_patch/models/0_wd4e5-0.15")
...
#####7-5                shutil.copy(sourse_path, pick_out)
...
if __name__ == '__main__':
    jpgFilePath = r"/root/linjian/darknet_patch/video/T1/"
    txt = open(r'/root/linjian/darknet_patch/video/T1/T-1_dom-HP.txt','a')
    out = jpgFilePath+r'T-1-pick_patches'
    all_img = r"/root/linjian/darknet_patch/raw_data/Validation/HP"
#    all_img = jpgFilePath+r'test-patches'
    i = 0
    num = 0
    ...
    for jpg_list in img_list:
        ...
        if image is None:
            ...
            number, HP, YT= eval_video(256, 180, image_tf, find_obj, out, i)
            txt.write(str(HP)+' '+str(YT)+'\n')
            if number == 1:
                num +=1
