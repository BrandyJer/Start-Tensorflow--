...
#            loss =  tf.nn.sparse_softmax_cross_entropy_with_logits(labels=labels, logits=logits)
            loss =  tf.nn.weighted_cross_entropy_with_logits(targets=labels, logits=logits,  pos_weight=1.5)
...
#            opt.minimize(total_loss, global_step=global_step)
            ...
            train_writer = tf.summary.FileWriter('/root/linjian/darknet_0/models/try-linjian/lj_data/A-0/loss_wd4e5_lj-0.5', sess.graph)
            sess.run(init)
...
#                saver.restore(sess, model_file)
                model_file=tf.train.get_checkpoint_state('./models/try-linjian/JZ_data/0_lj-wd8e5-0.01')
...
#                    checkpoint_path = os.path.join('/root/classify/model', 'dp15_model.ckpt')
                    checkpoint_path = os.path.join('/root/linjian/darknet_0/models/try-linjian/lj_data/A-0/loss_wd4e5_lj-0.5', 'model.ckpt')#save model path
