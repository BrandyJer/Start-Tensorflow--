...
if __name__ == '__main__':
    ...
    txt = open(r'/root/linjian/darknet_0/video/video2_ffmpegdiff.txt','a')
...
##    out = videoFilePath+'video2_img_init'
##    out_diff = videoFilePath+'video2_img_diff'
...
#        print(image.shape)
#        cv2.imwrite(os.path.join(out, str(i)+'-'+time_str+'.jpg'), image)
        print("============== "+str(i)+" ============")
        if i == 0:
#            last_gray = gray_img
            last_BGR = image
...
#            frameDelta_gray = cv2.AbsDiff(last_gray, gray_img)
            ...
            mean_BGR = np.mean(frameDelta_BGR, axis=2)
            BGR_nonZero = cv2.countNonZero(mean_BGR)
            gray_nonZero = cv2.countNonZero(frameDelta_gray)
#            print(frameDelta_gray.shape)
#            cv2.imwrite(os.path.join(out_diff, str(i)+'-'+time_str+'.jpg'), frameDelta_BGR)
            print("-----"+str(mean_gray)+"-----")
            print("-----"+str(BGR_nonZero)+"-----")
            print("-----"+str(gray_nonZero)+"-----")
            txt.write(str(mean_gray)+' '+str(BGR_nonZero)+' '+str(gray_nonZero)+' '+time_str+'\n')
#            last_gray = gray_img
            last_BGR = image
            if i == 9043:
                break
