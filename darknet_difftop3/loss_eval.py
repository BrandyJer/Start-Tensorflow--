...
from loss_net import tiny_darknet
...
#            loss_eval =  tf.nn.sparse_softmax_cross_entropy_with_logits(labels=labels_eval, logits=logits_eval)
            loss_eval =  tf.nn.weighted_cross_entropy_with_logits(targets=labels_eval, logits=logits_eval,  pos_weight=1.5)
            logits_eval = tf.nn.sigmoid(logits_eval) 
            saver = tf.train.Saver(tf.all_variables())
            init = tf.initialize_all_variables()
            sess = tf.Session(config=tf.ConfigProto(log_device_placement=False))
            sess = tf.Session()
            train_writer = tf.summary.FileWriter('/root/linjian/darknet_0/models/try-linjian/lj_data/A-0/loss_wd4e5_lj-0.5/eval', sess.graph)
            sess.run(init)
...
#            ckpt = tf.train.get_checkpoint_state("/root/JZ_test/darknet0_model")
            ckpt = tf.train.get_checkpoint_state("./models/try-linjian/lj_data/A-0/loss_wd4e5_lj-0.5")
#####            saver.restore(sess, ckpt.all_model_checkpoint_paths[-1])
#####            step = 0 
            for model in range(-60, 0):    #-55==>model-2000
#####            step+=1
...
#####                print("The "+str(step)+" iter eval loss:"+str(mse/float(num)))
                ...
                print("========Thre :"+str(thre)+'/'+str(model+60)+" _model eval ========")
                ...
                train_writer.add_summary(summary_str, model+60)
            train_writer.close()
