...
##33*2=66ms get one frame

#os.environ["CUDA_VISIBLE_DEVICES"] = ""
os.environ["CUDA_VISIBLE_DEVICES"] = "0"

def eval_video(w, h, im, im_pick, pick_out, num_th, time_jpg):
...
#            print l, logits_eval
            ...
            if p[1]<= thre:
                HP = 1
                cv2.imwrite(os.path.join(pick_out, str(num_th)+'-'+time_jpg+'.jpg'), im_pick)
...
#    txt = open(r'/root/linjian/darknet_0/video/dom-all_9999.txt','a')
    ...
    print "------------------------------"
    print(width_video, height_video)
...
#    fourcc = cv2.cv.CV_FOURCC('M', 'J', 'P', 'G') # motion-JPEG code
    out = videoFilePath+'pick2_test2-2500jz_YT0.925'
...
#            image = image.resize((int(1280/4), int(720/4)), Image.ANTIALIAS)
            print("============== "+str(i)+" ============")
#6.8            if image is None:
            if i == 10000:
                break
            else:
                number, HP, YT =  eval_video(320, 180, image_tf, image_cv, out, i, time_str)
