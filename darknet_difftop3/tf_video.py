...
#            tf.train.start_queue_runners(sess=sess)
            ckpt = tf.train.get_checkpoint_state(r"/root/linjian/darknet_0/models/try-linjian/lj_data/2_wd1e7-0.1")
...
#    fourcc = cv2.cv.CV_FOURCC('M', 'J', 'P', 'G') # motion-JPEG code
    out = videoFilePath+'pick_img-2650new0.88'
...
#        image = image.resize((int(1280/4), int(720/4)), Image.ANTIALIAS)
        ...
        if image is None:
            ...
            number =  eval_video(320, 180, image_tf, image_cv, out, i)
            if number == 1:
                num += 1
                print('--------------How many times HP showed :'+str(num))
#####                if last_i == i-1:
